...
CommandGroup, CommandInput, CommandItem, CommandList } from "@/components/ui/command";
...
export const ServerSearch = ({ 
  data 
    ...
    const onClick = ({ id, type}: { id: string, type: "channel" | "member"}) => { 
      ...
      if (type === "channel") { 
        return router.push(`/servers/${params?.serverId}/chanels/${id}`)
      }
    ...
    <>
      ...
      onClick={() => setOpen(true)}
        className="group px-2 py-2 rounded-md 
            flex items-center gap-x-2 w-full hover:bg-zinc-700/10 dark:hover:bg-zinc-700/50
            transition"
      >
